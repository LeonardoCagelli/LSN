...
const int n_cities = 34; // Number of cities
const double length = 10.0; // Length of the country
const int n_generation = 200; // Number of generations
...
void run_search(Country& country, Random& rnd, int world_size, int world_rank, bool exchange) {
    const size_t exchange_interval = 100;
    ...
    std::string name_best = "OUTPUT/best_rank_" + std::to_string(world_rank) + ".txt";
    std::string name_history_best = "OUTPUT/history_best_rank_" + std::to_string(world_rank) + ".txt";
    std::string name_history_half_avg = "OUTPUT/history_half_avg_rank_" + std::to_string(world_rank) + ".txt";
...
int main(int argc, char* argv[]) {
    ...
    Country country;
    if (world_rank == 0) {
        // Generate the country (e.g., circle or square)
        country = GenerateCountry(n_cities, 1, length, rnd);
    ...
    if (world_rank < NUM_SEARCHES) {
        run_search(country, rnd, world_size, world_rank, true);
    }
