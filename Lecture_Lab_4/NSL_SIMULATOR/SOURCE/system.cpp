...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ 
    ...
    // TO BE FIXED IN EXERCISE 6 ---------->   DONE
    double delta_E = 2.0 * _beta * _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() + _H );
    double p = exp(- delta_E);
    p = 1 / (1 + p); // probability with boltzmann 

    if(_rnd.Rannyu() < p) _particle(i).setspin(1);  
    else _particle(i).setspin(-1); 
    _naccepted++;       // the next move is always accepted
  ...
  {           // M(RT)^2
    if(_sim_type == 1){       // LJ system
      ...
      for(int j=0; j<_ndim; j++){
        shift(j) = _rnd.Rannyu(-1.0,1.0) * _delta; // the proposed move follows a uniform distribution in [-_delta;_delta)
      }

      // set a new actual position of particles 
      _particle(i).translate(shift, _side);  //Call the function Particle::translate

      if(this->metro(i)){ //Metropolis acceptance evaluation
        _particle(i).acceptmove(); // set _old_x equal to _x (the proposed move is accepted) 
        _naccepted++;
...
bool System :: metro(int i){ // Metropolis algorithm
  ...
  if(_sim_type == 1){       
    // simply compute the difference in energy between the previous position and the proposed one following the boltzmann distribution
    delta_E = this->Boltzmann(i,true) - this->Boltzmann(i,false);
  }
  else 
  {
    // compute difference in energy betwheen the previous configuration and the swipped one using Ising 1D model 
    // so considering the previous and the next particles
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream Primes(_input_path + "Primes");
  Primes >> p1 >> p2 ;
  Primes.close();
  int seed[4]; // Read the seed of the RNG
  ifstream Seed(_input_path + "seed.in");
  Seed >> seed[0] >> seed[1] >> seed[2] >> seed[3];
  _rnd.SetRandom(seed,p1,p2);

  ofstream couta(_output_path + "acceptance.dat"); // Set the heading line in file ../OUTPUT/acceptance.dat
  couta << "#   N_BLOCK:  ACCEPTANCE:" << endl;
  couta.close();

  ifstream input(_input_path + "input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      cerr << "PROBLEM Property: "<< property <<" | ";
  ...
  // I have implementd this part to save averages  - - - Leonardo Cagelli

  if(std::filesystem::exists(_input_path + "info.dat")){    // check if info.dat exists
    // info.dat is a specific file created by me to give additional information to the simulation
    input.open(_input_path + "info.dat");   

    while( ! input.eof() ){
      input >> property;
      if(property == "SAVE_AVERAGES") _save_averages = true;
    }
    input.close();
  }
...
void System :: initialize_velocities(){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open(_input_path + "CONFIG/velocities.in");
    if(cinf.is_open()){
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  ifstream input(_input_path + "properties.dat");
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp(_output_path + "potential_energy.dat");
        ...
        index_property++;
        // TO BE FIXED IN EXERCISE 7 --> DONE (TO CHECK)
        _vtail = (8 * M_PI * _rho) * (1.0 / (9 * pow(_r_cut, 9)) - 1.0 / (3 * pow(_r_cut, 3)));  //epsilon = 1
      } else if( property == "KINETIC_ENERGY" ){
        ofstream coutk(_output_path + "kinetic_energy.dat");
        ...
        ofstream coutt(_output_path + "total_energy.dat");
        ...
        ofstream coutte(_output_path + _temperature_path);
        ...
        ofstream coutpr(_output_path + "pressure.dat");
        ...
        index_property++;
        // TO BE FIXED IN EXERCISE 7 --> DONE (TO CHECK)
        _ptail = 32 * M_PI * _rho * (1.0 / (9 * pow(_r_cut, 9)) - 1.0 / (6 * pow(_r_cut, 3)));
      } else if( property == "GOFR" ){
        ofstream coutgr(_output_path + "gofr.dat");
        ...
        ofstream coutpr(_output_path + "magnetization.dat");
        ...
        ofstream coutpr(_output_path + "specific_heat.dat");
        ...
        ofstream coutpr(_output_path + "susceptibility.dat");
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(_output_path + "CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open(_output_path + "CONFIG/config.spin");
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open(_output_path + "CONFIG/config_" + to_string(nconf) + ".xyz");
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open(_output_path + "CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open(_input_path + "CONFIG/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open(_input_path + "CONFIG/config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: measure(){ // Measure properties
  ...
  double pressure_temp=0.0; // temporary variable for pressure computation 
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        // GOFR ... TO BE FIXED IN EXERCISE 7 --> DONE (TO CHECK)
        if(_measure_gofr && dr < _halfside.min()){
          bin = int(dr / _bin_size);
          if(bin < _n_bins) _measurement(_index_gofr + bin) += 2; // count pairs (i,j) and (j,i) symmetry
          else std:cerr<<"ERROR - Index out of range during GOFR computation!\n";
        }
        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          
          // PRESSURE ... TO BE FIXED IN EXERCISE 4 ---> DONE (TO CHECK)
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 4 --> DONE (to checK)
  if (_measure_pressure){
    // pressure_temp = _rho * _temp + (1.0/( _volume * 3.0)) * 48 * (pressure_temp / double(_npart));
    pressure_temp = _rho * (2.0 / 3.0) * kenergy_temp + 16.0 * virial / (double(_volume));
    _measurement(_index_pressure) =  pressure_temp;
  }

// MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6 -->  DONE (TO CHECK)
  if(_measure_magnet){
    for(size_t i{};i<_npart;i++) magnetization += _particle(i).getspin();
...
// TO BE FIXED IN EXERCISE 6 --> DONE (TO CHECK)
  if(_measure_cv){
    for(size_t i{};i<_npart;i++){
      double c1 = double(_particle(i).getspin());
      double c2 = double(_particle(this->pbc(i+1)).getspin());
      cv_temp += (- _J * c1 * c2 - 0.5 * _H * (c1 + c2));
    }
    _measurement(_index_cv) =  pow(cv_temp, 2);
  }

// SUSCEPTIBILITY ////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6 --> DONE (TO CHECK)
  if (_measure_chi) {
    for(size_t ind{};ind<_npart;ind++) chi_temp += _particle(ind).getspin();
    chi_temp = (_beta * pow(chi_temp, 2)) / double(_npart);
...
void System :: averages(int blk){
  ...
  // Normalization of the g(r)
  if (_measure_gofr){
    for(int i{_index_gofr};i<_index_gofr+_n_bins;i++){
      bin++;
      double bin_volume = (4.0 / 3.0) * M_PI * (pow((bin + 1) * _bin_size, 3) - pow(bin * _bin_size, 3));
      double ideal_gas_count = _rho * _npart * bin_volume;
      _block_av[i] /= ideal_gas_count;
    }
  }

  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(_output_path + "potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(_output_path + "kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(_output_path + "total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(_output_path + _temperature_path, ios::app);
  ...
  // TO BE FIXED IN EXERCISE 4 --> DONE (TO CHECK)
  if (_measure_pressure){
    coutf.open( _output_path + "pressure.dat",ios::app);
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 7 --> DONE (TO CHECK)
  if (_measure_gofr && blk == _nblocks - 1){
    bin = 0;
    coutf.open( _output_path + "gofr.dat",ios::app);
    for (int i{_index_gofr};i<_index_gofr+_n_bins;i++){
        sum_average = _global_av(i);
        sum_ave2 = _global_av2(i);
        coutf << bin * _bin_size      // distance                                               
  ...
  // TO BE FIXED IN EXERCISE 6 --> DONE (TO CHECK)
  if (_measure_magnet){
    coutf.open( _output_path + "magnetization.dat",ios::app);
  ...
  // TO BE FIXED IN EXERCISE 6    -----------> DONE  (to check)
  if (_measure_cv){
    coutf.open( _output_path + "specific_heat.dat",ios::app);
    average  = (pow(_beta, 2) * (_average[_index_cv] - pow(_average[_index_tenergy] * double(_npart), 2))) / double(_npart);
    _global_cv += average;
    _global_cv2 += pow(average, 2);
    sum_average = _global_cv;
    sum_ave2 = _global_cv2;
    coutf << setw(12) << blk
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  // TO BE FIXED IN EXERCISE 6 --> DONE (TO CHECK)
  if (_measure_chi){
    coutf.open( _output_path + "susceptibility.dat",ios::app);
  ...
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
  coutf.open( _output_path + "acceptance.dat",ios::app);
...
// my function to write averages in file.dat
void System :: write_averages(const std::string& path){
  ofstream coutf;
  coutf.open(path, std::ios::app);
  
  if(coutf.is_open()){
    for(size_t i{}; i<_average.n_elem; i++){
      coutf<<_average(i)<<" ";
    }
    coutf<<endl;
...
void System :: setInputPath(const std::string& path){
  this->_input_path = path;
  return;
}

void System :: setOutputPath(const std::string& path){
...
void System :: printPath(){
  ...
  std::cerr << "output path: "<<_output_path<<"\n";
  return;
}

void System :: SetTemperature(double temp){
  _temp = temp;
}


// function to read the final temperature from an output file .dat

void System :: setEquilibriumTemp(const std::string& path){
  std::ifstream input(path);
  if(!input){
    std::cerr<<"Error: readTemperature() function can't open file "<<path<<"!\n";
    return;
  }
  ...
  _temp = std::stod(tmp);
  std::cerr<<"Temp: "<<_temp<<std::endl;
  return;
}

// Set the path temperature for output file 
// I created this function for equilibration 
void System :: setTemperaturePath(const std::string& path){
  _temperature_path = path;
}
